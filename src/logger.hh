...
class Logger {
  ...
  public:
    ...
    static void successLog(const std::string &message) {
        if (vb == Verbosity::silent)
            return;
        std::cout << "\n[" << GREEN << "LOG" << RESET << "]   " << message;
        f_flush(force_f);
    }

    static void failLog(const std::string &message,
                        const char *exception = nullptr) {
        std::cout << "\n[" << RED << "LOG" << RESET << "]   " << message;
        if (vb == Verbosity::verbose || vb == Verbosity::debug) {
            std::cout << "\n[" << RED << "EXCEPTION" << RESET << "] ";
            std::cout << (exception ? exception : "unknown exception.")
                      << std::endl;
        }
        f_flush(force_f);
    }
    ...
    static void print_src_and_hdr(bool print_as_toml_array) {
        ...
        if (print_as_toml_array) {
            ...
            for (const auto &[_, src] : sources) {
                std::cout << "  \"" << readable_path(src.path) << "\", "
                          << std::endl;
            }
            if (!headers.empty()) {
                std::cout << BLUE << "\n######## headers ########" << RESET
                ...
                for (const auto &[_, hdr] : headers) {
                    std::cout << "  \"" << readable_path(hdr.path) << "\", "
                              << std::endl;
                }
            }
        } else {
            ...
            for (const auto &[_, src] : sources) {
                ...
                std::cout << "[" << BLUE << num << RESET << "]"
                          << std::string(width - num.length(), ' ') << " "
                          << readable_path(src.path) << std::endl;
                i++;
            }
            std::cout << "[" << CYAN << "LOG" << RESET << "] " << " " << (i - 1)
                      << " sources." << std::endl;
            if (!headers.empty()) {
                std::cout << "[" << CYAN << "headers" << RESET << "] "
                          << std::endl;
                for (const auto &[_, hdr] : headers) {
                    std::string num = std::to_string(j);
                    std::cout << "[" << BLUE << num << RESET << "]"
                              << std::string(width - num.length(), ' ') << " "
                              << readable_path(hdr.path) << std::endl;
                    j++;
                }
                std::cout << "[" << CYAN << "LOG" << RESET << "] " << " "
                          << (j - 1) << " headers." << std::endl;
            }
            std::cout << "[" << CYAN << "LOG" << RESET << "] "
                      << " total: " << (i + j - 2) << " files." << std::endl;
        }
    }
};
