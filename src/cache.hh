...
void cleanTemp(const fs::path &tempPath) {
    try {
        ...
        Logger::warningLog("clean up error while removing: \"" +
                           readable_path(tempPath) + "\"");
    }
}
...
void save_cache(const fs::path &cachePath) {
    ...
    try {
        ...
        Logger::failLog("save_cache(): failed to write \"" +
                            readable_path(cachePath) + "\"",
                        e.what());
        cleanTemp(tempPath);
    } catch (const fs::filesystem_error &e) {
        Logger::failLog("save_cache(): failed to rename temporary file",
                        e.what());
        cleanTemp(tempPath);
    }

    for (const auto &inc : headers) {
        Logger::debug("cached header: " + readable_path(inc.second.path) +
                      " | hash: " + std::to_string(inc.second.hash));
    }
}
...
void load_cache(const fs::path &cachePath) {
    ...
    try {
        ...
        Logger::failLog("load_cache(): failed to read cache at \"" +
                            readable_path(cachePath) + "\"",
                        e.what());
    }
}
